...
echo "### compile (compile-time specs/tags) + run (runtime specs) + aggregate …"
#   `clean` so the compile-time-emitted specs (`param`, and on Scala 2 also
#   `spec{}`/`bundle`) regenerate; `runMain Main` then emits the runtime specs.
( cd "$HERE" && "$SBT" clean "runMain streamproc.design.top.Main" )
